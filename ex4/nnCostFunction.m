...
for k=1:num_labels,
	Jvec(k) = (-1/m)*(sum(Y(:,k).*log(H(k,:)') + (1-Y(:,k)).*log(1-H(k,:)')));
end
...
for t=1:m,
 ...
 % Hidden Layer Gradient 
	d_2 = (Theta2(:,2:end)'*d_3).*sigmoidGradient(z_2); % 26x1

 % Partway Gradient Calculation, L = 2 
	D_2 = D_2 + (d_3 * a_2'); % 10x26
 % Partway Gradient Calculation, L = 1
	D_1 = D_1 + (d_2 * a_1'); % 25x401
end
